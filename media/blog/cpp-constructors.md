...
`std::unique_ptr` abstraction, and they asked “_What’s a safest `std::unique_ptr`?_ My main problem
was (still is) that `std::unique_ptr` is not safe at all, as it can be initialized with `nullptr`.
...
I’m going to explain why in a series of blog article, starting with this one. Let’s go.
...
* [How would do that in Rust?](#how-would-do-that-in-rust)
...
_designated initializer_ to initialize a `struct` directly while you’re declaring it. Some examples:
...
what’s the problem by reading on.
...
> talk about it. I also have to admit I never completely recall all the rules — C++.

Because C++ is fucking complicated, the following program doesn’t do what you think it does, if
you’re not experienced in C++ enough (and even then, sometimes, it doesn’t depend on what you know):
...
- It’s hard to tell when we are going to copy something. Think of a more complex data structure,
  ...
  while your code doesn’t explicitly copies anything.
- Because copy constructors and move constructors are subject to compiler optimizations, we don’t
  really know when the [(N)RVO] optimization is going to be applied — we can have good confidence
...
We’ve established that C++ makes it _very easy_ to write code which has hidden copies all over the
place because of the existence of copy constructors and that value semantics depends on it. Let’s
go on with a problem I have with constructors that is as bad to me.
...
that. Let’s enumerate a few of them:
...
really bad runtime undefined behavior… or you will have to handle the invariant at every call, which
doesn’t seem like a lot of fun — and it’s dangerous because you can forget to check it. Also, you
will check it every time you want to use `non_zero`, which seems silly.
...
constructors are broken, because not all values can be constructed via C++ constructors. Try to
spend a few minutes and think about it. Can you implement a `struct NonZero` with an API exposing
**only constructors** to create and initialize non-zero values? There are two possibilities:
...
- Either you, indeed, use a constructor, but because you want to be able to prevent people from
  ...
  **only be used in exceptional failure situations**, and this is not one of them.
...
> I will dedicate the second blog article of this series to C++ exceptions as I have so much
...
```cpp
struct NonEmpty {
  // prevent people from creating default non-empty values; it makes no sense
  NonEmpty() = delete;

  NonEmpty(uint32_t value): _wrapped(value) {
...
As you can see, having a `NonEmpty` with this definition doesn’t mean the value cannot be `0`.
Example:
...
This code compiles fine and will either aborts at runtime on an assert or just silently do something
...
apply that advice to themselves.

All that to say: don’t do this. Don’t think constructors are a good idea to construct types in all
possible situations. Actually, I think constructors are not a good idea _at all_. You might
wonder:
...
cannot create a value without using a constructor, and since constructors fail to encode failible
constructions, I’m going to use a private constructor to build my `NonZero` and _not_ check the
invariant. Then, using a `static` method, I can return a more typed object to encode a possible
failure, by checking the invariant _before_ creating the object.
...
struct NonEmpty {
  ...
  // static method used to create a NonEmpty; the only way to create one
  // using the public interface
  std::optional<NonEmpty> from_u32(uint32_t value) {
    std::optional<NonEmpty> r;
...
So let me explain a bit all this code. First, the `NonEmpty(uint32_t)` constructor is private. It
has to be private because it doesn’t enforce the invariant. It allows to construct any `NonEmpty`.
We need it because C++ requires you to use a constructor to create a value of type `NonEmpty`.
...
`NonEmpty`. As you can see, it returns a `std::optional<NonEmpty>`, which means that it can fail.
...
initialize the `NonEmpty`, and return the whole thing.
...
But we’re not completely done. We’ve used the `std::optional` standard type. How are we supposed
to use that type? Looking at the official documentation, we get use the [`operator*` or
...
have been done in a much more safe way, by, for instance, providing you with some combinators to
...
# How would do that in Rust?

In Rust, we have _exhaustive static dispatch_. `std::optional` is called `Option` and Rust
doesn’t have constructors _at all_. No hidden copies — if you want to copy something like in a
C++ copy constructor, you have to explicitly do it — we use the `Clone` trait for that, with the
`clone()` method. The default value semantics is the _move_ semantics, so passing a `Vec<T>` to a
function will just move it around, with zero copy.
...
Here, the type of `port` is `NonEmpty`. `Option::unwrap_or` might be implemented like this:
...
etc. I’ll let you dig by yourselves.

So, we’re hitting the end of the article. It might be a lot of information; sorry. Yes, I already
see coming an army of C++ developers insulting me and stating that _good programmers know the fuck
they are doing and no one needs that static bullshit_. People who say that _a good programmer
doesn’t do segfaults_ or _a good programmer knows how to use gdb / lldb_. I’ve had discussions with
people like that on IRC, Twitter and even around beers with friends years ago. This article is
written for people  willing to leave their comfort zone, question their knowledge and take some
hindsight. I truly think, as an experienced software engineer, that the key to evolve in computer
science is to _always_ accept that something that is true today is likely to be deprecated
tomorrow. When you learn how to live with that statement, everything is much, much clearer… and
easy.

Next time, I’ll bring you on a tour with me around the concept of _inclusion polymorphism_, which is
most of the time called _inheritance_ in the world of OOP languages. But we’ll still stick to C++.
;)
