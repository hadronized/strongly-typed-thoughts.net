...
* [Rank-N quatification](#rank-n-quatification)
* [Kinds](#kinds)
* [GAT](#gat)
...
# Rank-N quatification

It’s for sure one among the two feature I miss the most from Haskell. Rank-N quantification is a
...
For those not used to it who those having no idea what rank-N quantification are, let me explain
...
allow us to do what we want to do: `T` shouldn’t be monomorphized _at the same rank as_ `F`, because
...
But it’s rarely needed and I struggle to find a real usecase for them (but they are!). From my
...
[GAT](https://github.com/rust-lang/rust/issues/44265) are a bit akin to kinds in the sense that they
...
Something I want a lot too and haven’t been discussed a lot (I might write an RFC for that because
I want them very badly). What it means is that:
